...
namespace pangolin {
	template <typename T>
		struct Points : public Renderable, public Interactive
	{
		Points(const std::vector<Eigen::Matrix<T,3,1>>& listOfPoints, int pointSize = 2 ) : _listOfPoints(listOfPoints)
		{
		...
		const std::vector<Eigen::Matrix<T,3,1>> _listOfPoints;
		int _pointSize;
	};
}
